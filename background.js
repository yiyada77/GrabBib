...
function acm() {
    ...
    let targetFile = 'custom-bibtex'
    let format = 'bibTex'
    ajax.open('get', `https://dl.acm.org/action/exportCiteProcCitation?dois=${dois}&targetFile=${targetFile}&format=${format}`);
    ajax.send(null);
    ajax.onreadystatechange = function() {
        if (ajax.readyState == 4 && ajax.status == 200) {
            /**
            ...
            if (rawObj.type === 'PAPER_CONFERENCE' || rawObj.type === 'CHAPTER') {
                ...
                resArr.push(`year = {20${series.match(/\d+(.\d+)?/g)}},\n`)
                resArr.push(`series = {${series}}\n`)
                    // resArr.push(`year = {20${rawObj['collection-title'].match(/\d+(.\d+)?/g)}},\n`)
                    // resArr.push(`series = {${rawObj['collection-title']}}\n`)
            }
            if (rawObj.type === 'ARTICLE') {
                resArr.push(`@article{${rawObj.id},\n`)
                resArr.push(`title = {${rawObj.title}},\n`)
